...
import numpy as np
from diff_attn import MultiheadDiffAttn
...
class AKT(nn.Module):
    ...
    def forward(self, q_data, qa_data, target, pid_data=None):
        ...
        output = self.out(concat_q)#output.shape:[bs,seq,1]
        labels = target.reshape(-1)#target,shape:[bs,seq] labels.shape:[bs*seq]
        m = nn.Sigmoid()
        preds = (output.reshape(-1))  # logit  展到一维平面 flatten
        mask = labels > -0.9 #常用-1填充
        ...
        if model_type in {'akt'}:
            self.blocks_1 = nn.ModuleList([
                TransformerLayer(d_model=d_model, d_feature=d_model // n_heads,
                                 d_ff=d_ff, dropout=dropout, n_heads=n_heads, kq_same=kq_same, depth = i)
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature,
                 d_ff, n_heads, dropout, kq_same, depth):
        super().__init__()
        """
        ...
        self.mask_diff_mha = MultiheadDiffAttn(
            embed_dim = d_model, depth = depth, num_heads = 8)
