...
def train(net, params,  optimizer,  q_data, qa_data, pid_data,  label):
    ...
    for idx in range(N):
        ...
        target_1 = np.floor(target) #向下取整 就把填充0变成了填充-1
        ...
        pred = pred.detach().cpu().numpy()  # (seqlen * batch_size, 1)已展平
        loss.backward()
        ...
        target = target_1.reshape((-1,))# 展平 target_.shape:[bs,seq] target:[bs*seq]
