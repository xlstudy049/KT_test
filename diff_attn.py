...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    return (
        ...
        #.expand() 没有复制数据，它只是通过广播创建了一个新的视图。
        #.repeat() 创建了一个新的张量并复制了数据，因此在内存上会比 .expand() 更加昂贵。
        # 当一个张量进行了 转置（transpose permute），会导致张量变成不连续的
        # .reshape()能处理连续和非连续的张量
    )
...
class MultiheadDiffAttn(nn.Module):
    def __init__(
        ...
        # .normal_ 原地操作，初始化为一个高斯分布（0，1）的值
    ...
    def forward(
        ...
        attn_weights = torch.nan_to_num(attn_weights) #将nan值转换 0 值
